...
class Review(models.Model):
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='motorcycle_reviews', verbose_name="المستخدم")
    motorcycle = models.ForeignKey(Motorcycle, on_delete=models.CASCADE, related_name='reviews', verbose_name="الدراجة")
    ...
    class Meta:
        ...
        ordering = ['-created_at']
        constraints = [
            models.UniqueConstraint(fields=['user', 'motorcycle'], name='unique_user_motorcycle_review'),
            models.CheckConstraint(condition=models.Q(rating__gte=1, rating__lte=5), name='review_rating_between_1_and_5'),
        ]

    def __str__(self):
        return f"{self.user.username} - {self.motorcycle.name} ({self.rating}/5)"
...
class SentEmail(models.Model):
    ...
    sender = models.ForeignKey(
        ...
        related_name='sent_site_emails', verbose_name='المرسل من الإدارة'
    )
    recipient_user = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True,
        related_name='received_site_emails', verbose_name='المستخدم المستلم'
    )
    recipient_email = models.EmailField(verbose_name='البريد المستلم')
    subject = models.CharField(max_length=255, verbose_name='عنوان الرسالة')
    message = models.TextField(verbose_name='نص الرسالة')
    status = models.CharField(max_length=10, choices=STATUS_CHOICES, default='sent', verbose_name='الحالة')
